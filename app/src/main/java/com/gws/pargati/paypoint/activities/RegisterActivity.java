...
public class RegisterActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private void userSignup()
    {
        ...
        if(mobile.isEmpty())
        {
            etMobileRegister.setError("Mobile Number is required");
            etMobileRegister.requestFocus();
            return;
        }

        if(mobile.length() < 10)
        {
            etMobileRegister.setError("Enter a valid Mobile Number");
            etMobileRegister.requestFocus();
            return;
        }
        ...
        {
            etPasswordRegister.setError("Password length must be atleast 8 characters");
            etPasswordRegister.requestFocus();
            return;
        }
